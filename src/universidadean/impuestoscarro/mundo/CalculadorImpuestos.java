...
public class CalculadorImpuestos {
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        double valorImpuesto=rango.darPorcentaje()/100;
        if(!descProntoPago && !descServicioPublico && !descTrasladoCuenta) {
            pago=precio*valorImpuesto;
        }

        if(descProntoPago && !descServicioPublico && !descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = pago*(PORC_DESC_PRONTO_PAGO/100);
        ...
        if(!descProntoPago && !descServicioPublico && descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = pago*(PORC_DESC_TRASLADO_CUENTA/100);
            System.out.format("2: %f\n ",descuento);
            pago=pago-descuento;
        }
        if(descProntoPago && descServicioPublico && !descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = pago*(PORC_DESC_PRONTO_PAGO/100)+50000;
            System.out.format("2: %f\n ",descuento);
            pago=pago-descuento;
        }
        if(descProntoPago && !descServicioPublico && descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = pago*(PORC_DESC_PRONTO_PAGO/100)+pago*(PORC_DESC_TRASLADO_CUENTA/100);
            System.out.format("2: %f\n ",descuento);
            pago=pago-descuento;
        }
        if(!descProntoPago && descServicioPublico && descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = 50000+pago*(PORC_DESC_TRASLADO_CUENTA/100);
            System.out.format("2: %f\n ",descuento);
            pago=pago-descuento;
        }
        if(descProntoPago && descServicioPublico && descTrasladoCuenta) {
            pago=precio*valorImpuesto;
            System.out.format("1: %f\n ",pago);
            double descuento = pago*(PORC_DESC_PRONTO_PAGO/100)+50000+pago*(PORC_DESC_TRASLADO_CUENTA/100);
            System.out.format("2: %f\n ",descuento);
            pago=pago-descuento;
        }
    ...
    public Vehiculo buscarVehiculoPorLinea(String linea) {
        ...
        for(Vehiculo l : vehiculos) {
            if (l.darLinea().equalsIgnoreCase(linea)) {
                buscado = l;
            }
        }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
       ...
       // Vehiculo masAntiguo = vehiculos[0];

        // TODO: Buscar el vehÃ­culo mÃ¡s caro del arreglo de vehÃ­culos
       // for (Vehiculo a : vehiculos) {
         //   if (a.darAnio() > masAntiguo.darAnio())
        //        masAntiguo = a;
        //}
